from utils import INTERACTIVE_CONTROL_TYPE_NAMES, DOCUMENT_CONTROL_TYPE_NAMES, INFORMATIVE_CONTROL_TYPE_NAMES, DEFAULT_ACTIONS, THREAD_MAX_RETRIES
from views import TreeElementNode, ScrollElementNode, Center, BoundingBox, TreeState, TextElementNode, App
from uiautomation import Control, ImageControl, ScrollPattern, WindowControl, Rect, GetRootControl, PatternId
from utils import random_point_within_bounding_box, get_screen_size, is_app_browser, get_screenshot
...
class Tree:
    ...
    def get_state(self, active_app: App, other_apps: list[App]) -> TreeState:
        root = GetRootControl()
        other_apps_handle = set(
            map(lambda other_app: other_app.handle, other_apps))
        apps = list(filter(
            lambda app: app.NativeWindowHandle not in other_apps_handle, root.GetChildren()))
        del other_apps_handle
        if active_app:
            apps = list(filter(lambda app: app.ClassName != 'Progman', apps))
        interactive_nodes, scrollable_nodes, dom_informative_nodes = self.get_appwise_nodes(
            apps=apps)
        ...
        if self.dom:
            scroll_pattern: ScrollPattern = self.dom.GetPattern(
    ...
    def get_nodes(self, node: Control, is_browser: bool = False) -> tuple[list[TreeElementNode], list[ScrollElementNode], list[TextElementNode]]:
        ...
        # print(f'DOM interactive nodes:{len(dom_interactive_nodes)}')
        # print(f'DOM informative nodes:{len(dom_informative_nodes)}')
        # print(f'Scrollable nodes:{len(scrollable_nodes)}')
    ...
    def annotated_screenshot(self, nodes: list[TreeElementNode]) -> Image.Image:
        ...
        def draw_annotation(label, node: TreeElementNode):
            ...
            draw.rectangle(
                [(label_x1, label_y1), (label_x2, label_y2)], fill=color)
            draw.text((label_x1 + 2, label_y1 + 2), str(label),
        ...
        padded_screenshot.save("padded_screenshot.png")
